...
class CBaseHpsInterface {
     ...
     public function callHttpRequestWithLog($urlPath,$arrParas,$method,$timeout=3)
     {
         ...
         $tEnd = Log::getLogTime();
         Log::writeLog('CBaseHpsInterface::'. $method .' ret[' . $data . ']  url[' . $url . ']','debug', $tEnd-$tBegin);
         if(!$data)
         {
             Log::writeLog('CBaseHpsInterface::'.$method .'  Call Error.  url[' . $url . ']', 'error');
             return false;
         }
